				  ...
				  void *user_data)
{
  ...
  else if (strcmp(method, "PurgeCacheEntry") == 0)
  {
	  if (!dbus_message_iter_init(message, &args)) {
		result = false;
		  my_syslog(LOG_ERR, _("%s/%s: Message has no arguments!"), __FILE__, __FUNCTION__);
	  } else if (DBUS_TYPE_STRING != dbus_message_iter_get_arg_type(&args)) {
		result = false;
		  my_syslog(LOG_ERR, _("%s/%s: Argument is not string!"), __FILE__, __FUNCTION__);
	  } else {
		 dbus_message_iter_get_basic(&args, &purge_fqdn);
		 my_syslog(LOG_INFO, _("%s/%s: Got a method(PurgeCacheEntry) call of %s"), __FILE__, __FUNCTION__, purge_fqdn);
		  if ((count=cache_purge_by_name(purge_fqdn))>0) {
		result = true;
		} else {
		result = false;
		}
	...
	dbus_message_append_args(reply, DBUS_TYPE_BOOLEAN, &result, DBUS_TYPE_INVALID);	
	  ...
	  if (!dbus_connection_send(connection, reply, NULL)) {
		  my_syslog(LOG_ERR, "%s/%s: Out Of Memory while sending DBus reply!", __FILE__, __FUNCTION__);
	  }
	  dbus_connection_flush(connection);

	 my_syslog(LOG_DEBUG, _("%s/%s: %d entries purged, reply sent(%d)"), __FILE__, __FUNCTION__, count, result);
