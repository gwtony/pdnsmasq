...
int main(int argc, char **argv)
{
	...
	if (daemon->tftp_unlimited || daemon->tftp_interfaces)
		die(_("TFTP server not available: set HAVE_TFTP in src/config.h"),
				NULL, EC_BADCONF);
#endif
	...
	if (option_bool(OPT_CONNTRACK)
		...
		die(_("Cannot use --conntrack AND --query-port"), NULL,
				EC_BADCONF);
#else
	if (option_bool(OPT_CONNTRACK))
		die(_
	...
	if (daemon->max_logs != 0)
		die(_("asychronous logging is not available under Solaris"), NULL,
				EC_BADCONF);
#endif

#ifdef __ANDROID__
	if (daemon->max_logs != 0)
		die(_("asychronous logging is not available under Android"), NULL,
				EC_BADCONF);
#endif
	...
	if (daemon->ra_contexts || option_bool(OPT_RA)) {
		...
		if (!daemon->ra_contexts)
			...
			for (context = daemon->ra_contexts; context->next;
...
//#endif
	...
	if (daemon->radmin==1) {
		daemon->radmin_pid = fork();
		assert(daemon->radmin_pid>=0);
		if (daemon->radmin_pid==0) {
			close(err_pipe[1]);
			sprintf(uidstr, "%d", ent_pw->pw_uid);
			setenv("HELPERUID", uidstr, 1);
			ret = execl(LIBEXECDIR"/dnsmasq_radmin_helper", "{Remote admin helper}", NULL);
			syslog(LOG_ERR, "execl(%s): %s", LIBEXECDIR"/dnsmasq_radmin_helper", strerror(ret));
			abort();	/* Never */
		}
	}
	...
	if ((daemon->dhcp || daemon->dhcp6)
			&& (daemon->lease_change_command || daemon->luascript))
		daemon->helperfd =
			create_helper(pipewrite, err_pipe[1], script_uid, script_gid,
					max_fd);
#endif
	...
	if (daemon->port == 0)
		my_syslog(LOG_INFO, _("started, version %s DNS disabled"),
				VERSION);
	else if (daemon->cachesize != 0)
		my_syslog(LOG_INFO, _("started, version %s cachesize %d"), VERSION,
				daemon->cachesize);
	else
		my_syslog(LOG_INFO, _("started, version %s cache disabled"),
				VERSION);
		...
		for (; dhcp_tmp; dhcp_tmp = dhcp_tmp->next) {
			...
			if ((dhcp_tmp->flags & CONTEXT_DHCP) || family == AF_INET)
				my_syslog(MS_DHCP | LOG_INFO,
						...
						daemon->dhcp_buff, daemon->dhcp_buff3,
						daemon->namebuff);

			if (dhcp_tmp->flags & CONTEXT_RA_NAME)
				my_syslog(MS_DHCP | LOG_INFO,
						_("DHCPv4-derived IPv6 names on %s"),
						daemon->addrbuff);
			if (dhcp_tmp->
					flags & (CONTEXT_RA_ONLY | CONTEXT_RA_NAME |
...
static int read_event(int fd, struct event_desc *evp, char **msg)
{
	...
	if (!read_write
			(fd, (unsigned char *) evp, sizeof(struct event_desc), 1))
		return 0;
	...
	if (read_event(pipe, &ev, &msg))
		switch (ev.event) {
			case EVENT_RELOAD:
				...
				if (daemon->port != 0 && daemon->resolv_files
			...
			case EVENT_EXITED:
				my_syslog(LOG_WARNING,
						_("script process exited with status %d"), ev.data);
				break;
...
#endif
				syslog(LOG_DEBUG, "Kill radmin helper (%d) now.", daemon->radmin_pid);
				if (kill(daemon->radmin_pid, SIGTERM)==0) {
					waitpid(daemon->radmin_pid, NULL, 0);
					syslog(LOG_DEBUG, "radmin helper (%d) exited.", daemon->radmin_pid);
				} else {
					syslog(LOG_DEBUG, "kill() failed: %s", strerror(errno));
				}
...
void poll_resolv(int force, int do_reload, time_t now){
	...
	for (latest = NULL, res = daemon->resolv_files; res; res = res->next)
		if (stat(res->name, &statbuf) == -1) {
			...
			if (!res->logged)
				my_syslog(LOG_WARNING, _("failed to access %s: %s"),
						res->name, strerror(errno));
			res->logged = 1;
	...
	if (latest) {
		...
		if (reload_servers(latest->name)) {
			...
			if (!warned) {
				my_syslog(LOG_WARNING,
						_("no servers found in %s, will retry"),
						latest->name);
				warned = 1;
			}
		}
...
static int set_dns_listeners(time_t now, fd_set * set, int *maxfdp)
{
	...
	for (transfer = daemon->tftp_trans; transfer;
...
static void check_dns_listeners(fd_set * set, time_t now)
{
	...
	for (serverfdp = daemon->sfds; serverfdp; serverfdp = serverfdp->next)
		if (FD_ISSET(serverfdp->fd, set))
			reply_query(serverfdp->fd, serverfdp->source_addr.sa.sa_family,
					now);
	...
	if (daemon->port != 0 && !daemon->osport)
		for (i = 0; i < RANDOM_SOCKS; i++)
			if (daemon->randomsocks[i].refcount != 0 &&
				...
				reply_query(daemon->randomsocks[i].fd,
						daemon->randomsocks[i].family, now);
	...
	for (listener = daemon->listeners; listener; listener = listener->next) {
		...
		if (listener->tcpfd != -1 && FD_ISSET(listener->tcpfd, set)) {
			...
			if (confd == -1 ||
					getsockname(confd, (struct sockaddr *) &tcp_addr,
						&tcp_len) == -1)
				continue;
