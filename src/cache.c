...
/*
int cache_purge_by_name(char *name)
{
	int count=0;
	struct crec *next, **up, *crecp;
	unsigned short ins_flags = 0;

	for (crecp = hash_bucket(name); crecp; crecp = next)
	{
		next = crecp->hash_next;
...
int cache_purge_by_name(char *name)
{
	...
	now = dnsmasq_time()+86400*365; 

	crecp = cache_find_by_name(NULL, name, now, -1);
	if (crecp==NULL) {
		return -1;
	}
	for (; crecp; crecp = cache_find_by_name(crecp, name, now, -1)) {
		crecp->ttd=0;
	}
	return cache_scan_free(name, NULL, now, F_FORWARD );
}
